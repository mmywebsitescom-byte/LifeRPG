...
export interface Quest {
  ...
  estimatedTime: string; // e.g. "45m", "2h"
  xpReward: number;
  goldReward: number;
  attribute: AttributeType;
  attributeReward: number;
  dueDate: string; // YYYY-MM-DD
  dueTime?: string; // HH:mm format, e.g. "18:00"
  status: QuestStatus;
  repeat: QuestRepeat;
  createdAt: string;
  completedAt?: string;
...
export interface ToastNotification {
  id: string;
  type: 'success' | 'gold' | 'achievement' | 'error' | 'info';
  title: string;
  message?: string;
  duration?: number;
}

export interface CompletionAnimationPayload {
  questTitle: string;
  xpGained: number;
  goldGained: number;
  attribute: AttributeType;
